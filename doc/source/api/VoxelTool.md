...
`dst_mask_value` channel from the destination that will be used to select writable voxels.

`dst_writable_list` List of values the destination voxels must have in order to be written to. Values in that list must not exceed 65535. A very large amount of values can affect performance.
