...
[void](#)                                                                       | [do_blend_ball](#i_do_blend_ball) ( [Vector3i](https://docs.godotengine.org/en/stable/classes/class_vector3i.html) center, [int](https://docs.godotengine.org/en/stable/classes/class_int.html) radius, [int](https://docs.godotengine.org/en/stable/classes/class_int.html) strength )                                                                                                                                                                                                 
...
- [void](#)<span id="i_do_blend_ball"></span> **do_blend_ball**( [Vector3i](https://docs.godotengine.org/en/stable/classes/class_vector3i.html) center, [int](https://docs.godotengine.org/en/stable/classes/class_int.html) radius, [int](https://docs.godotengine.org/en/stable/classes/class_int.html) strength ) 

Smoothens out terrain by performing box blur.
`center` is position in the terrain that will be smoothed out.
`radius` is 8-bit unsigned integer radius of a sphere from the `center` where voxel values will be affected. Nothing is done with `radius` of 0.
`strength` is 8-bit unsigned integer of half the box blur length that will be sampled to calculate average voxel value. Higher `strength` results in more aggressive smoothing. Nothing is done with `strength` of 0.
Note 1: This is currently implemented only for terrain that uses SDF data (smooth voxels).
Note 2: Beware of using `radius` and `strength` values greater than 20 as the performance can drop quickly if this is called 60 times a second.
