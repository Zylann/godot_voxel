...
You may assign a new `StandardMaterial3D` in the first slot, and assign its `albedo_texture` to your texture. You may also check the `Vertex Color/Use as albedo` property, because this will allow the mesher to bake ambient occlusion on the edge of cubes.

![StandardMaterial3D for blocky terrain](images/material_blocky_game.png)
