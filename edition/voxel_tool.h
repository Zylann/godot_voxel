...
public:
	...
	uint64_t get_voxel(VOX_Vector3i pos) const;
	float get_voxel_f(VOX_Vector3i pos) const;
	...
	virtual void set_voxel(VOX_Vector3i pos, uint64_t v);
	virtual void set_voxel_f(VOX_Vector3i pos, float v);
	virtual void do_point(VOX_Vector3i pos);
	virtual void do_line(VOX_Vector3i begin, VOX_Vector3i end);
	virtual void do_circle(VOX_Vector3i pos, int radius, VOX_Vector3i direction);
	virtual void do_sphere(Vector3 center, float radius);
	virtual void do_box(VOX_Vector3i begin, VOX_Vector3i end);

	void sdf_stamp_erase(Ref<VoxelBuffer> stamp, VOX_Vector3i pos);

	virtual void copy(VOX_Vector3i pos, Ref<VoxelBuffer> dst, uint8_t channels_mask) const;
	virtual void paste(VOX_Vector3i pos, Ref<VoxelBuffer> p_voxels, uint8_t channels_mask, bool use_mask,
			uint64_t mask_value);
	...
	virtual void set_voxel_metadata(VOX_Vector3i pos, Variant meta);
	virtual Variant get_voxel_metadata(VOX_Vector3i pos) const;
...
protected:
	...
	virtual uint64_t _get_voxel(VOX_Vector3i pos) const;
	virtual float _get_voxel_f(VOX_Vector3i pos) const;
	virtual void _set_voxel(VOX_Vector3i pos, uint64_t v);
	virtual void _set_voxel_f(VOX_Vector3i pos, float v);
...
private:
	...
	uint64_t _b_get_voxel(Vector3 pos) {
		return get_voxel(VOX_Vector3i::from_floored(pos));
	}
	float _b_get_voxel_f(Vector3 pos) {
		return get_voxel_f(VOX_Vector3i::from_floored(pos));
	}
	void _b_set_voxel(Vector3 pos, uint64_t v) {
		set_voxel(VOX_Vector3i::from_floored(pos), v);
	}
	void _b_set_voxel_f(Vector3 pos, float v) {
		set_voxel_f(VOX_Vector3i::from_floored(pos), v);
	}
	...
	void _b_do_point(Vector3 pos) {
		do_point(VOX_Vector3i::from_floored(pos));
	}
	void _b_do_line(Vector3 begin, Vector3 end) {
		do_line(VOX_Vector3i::from_floored(begin), VOX_Vector3i::from_floored(end));
	}
	void _b_do_circle(Vector3 pos, float radius, Vector3 direction) {
		do_circle(VOX_Vector3i::from_floored(pos), radius, VOX_Vector3i::from_floored(direction));
	}
	void _b_do_sphere(Vector3 pos, float radius) {
		do_sphere(pos, radius);
	}
	void _b_do_box(Vector3 begin, Vector3 end) {
		do_box(VOX_Vector3i::from_floored(begin), VOX_Vector3i(end));
	}
	void _b_copy(Vector3 pos, Ref<RefCounted> voxels, int channel_mask) {
		copy(VOX_Vector3i::from_floored(pos), voxels, channel_mask);
	}
	void _b_paste(Vector3 pos, Ref<RefCounted> voxels, int channels_mask, int64_t mask_value) {
		// TODO May need two functions, one masked, one not masked, or add a parameter, but it breaks compat
		paste(VOX_Vector3i::from_floored(pos), voxels, channels_mask, mask_value > 0xffffffff, mask_value);
	}

	Variant _b_get_voxel_metadata(Vector3 pos) const {
		return get_voxel_metadata(VOX_Vector3i::from_floored(pos));
	}
	void _b_set_voxel_metadata(Vector3 pos, Variant meta) {
		return set_voxel_metadata(VOX_Vector3i::from_floored(pos), meta);
	}

	bool _b_is_area_editable(AABB box) const {
		return is_area_editable(Box3i(VOX_Vector3i::from_floored(box.position), VOX_Vector3i::from_floored(box.size)));
	}
...
protected:
	...
	struct TextureBlendSphereOp {
		...
		inline void operator()(VOX_Vector3i pos, uint16_t &indices, uint16_t &weights) const {
	...
	struct SdfOperation16bit {
		Op op;
		Shape shape;
		inline uint16_t operator()(VOX_Vector3i pos, uint16_t sdf) const {
