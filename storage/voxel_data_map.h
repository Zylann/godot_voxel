...
public:
	...
	static _FORCE_INLINE_ VOX_Vector3i voxel_to_block_b(VOX_Vector3i pos, int block_size_pow2) {
		return pos >> block_size_pow2;
	}

	_FORCE_INLINE_ VOX_Vector3i voxel_to_block(VOX_Vector3i pos) const {
		return voxel_to_block_b(pos, _block_size_pow2);
	}

	_FORCE_INLINE_ VOX_Vector3i to_local(VOX_Vector3i pos) const {
		return VOX_Vector3i(
	...
	_FORCE_INLINE_ VOX_Vector3i block_to_voxel(VOX_Vector3i bpos) const {
	...
	int get_voxel(VOX_Vector3i pos, unsigned int c = 0) const;
	void set_voxel(int value, VOX_Vector3i pos, unsigned int c = 0);

	float get_voxel_f(VOX_Vector3i pos, unsigned int c = VoxelBufferInternal::CHANNEL_SDF) const;
	void set_voxel_f(real_t value, VOX_Vector3i pos, unsigned int c = VoxelBufferInternal::CHANNEL_SDF);
	...
	void copy(VOX_Vector3i min_pos, VoxelBufferInternal &dst_buffer, unsigned int channels_mask) const;

	void paste(VOX_Vector3i min_pos, VoxelBufferInternal &src_buffer, unsigned int channels_mask, bool use_mask,
			uint64_t mask_value, bool create_new_blocks);

	// Moves the given buffer into a block of the map. The buffer is referenced, no copy is made.
	VoxelDataBlock *set_block_buffer(VOX_Vector3i bpos, std::shared_ptr<VoxelBufferInternal> &buffer);
	...
	VoxelDataBlock *get_block(VOX_Vector3i bpos);
	const VoxelDataBlock *get_block(VOX_Vector3i bpos) const;
	...
	// D action(VOX_Vector3i pos, D value)
	template <typename F>
	void write_box(const Box3i &voxel_box, unsigned int channel, F action) {
		...
		const VOX_Vector3i block_size(get_block_size());
		block_box.for_each_cell_zxy([this, action, voxel_box, channel, block_size](VOX_Vector3i block_pos) {
			VoxelDataBlock *block = get_block(block_pos);
			if (block != nullptr) {
				const VOX_Vector3i block_origin = block_to_voxel(block_pos);
				Box3i local_box(voxel_box.pos - block_origin, voxel_box.size);
				local_box.clip(Box3i(VOX_Vector3i(), block_size));
				block->get_voxels().write_box(local_box, channel, action, block_origin);
			}
		});
	}

	// action(VOX_Vector3i pos, D0 &value, D1 &value)
	template <typename F>
	void write_box_2(const Box3i &voxel_box, unsigned int channel0, unsigned int channel1, F action) {
		const Box3i block_box = voxel_box.downscaled(get_block_size());
		const VOX_Vector3i block_size(get_block_size());
		block_box.for_each_cell_zxy([this, action, voxel_box, channel0, channel1, block_size](VOX_Vector3i block_pos) {
			VoxelDataBlock *block = get_block(block_pos);
			if (block != nullptr) {
				const VOX_Vector3i block_origin = block_to_voxel(block_pos);
				Box3i local_box(voxel_box.pos - block_origin, voxel_box.size);
...
private:
	void set_block(VOX_Vector3i bpos, VoxelDataBlock *block);
	VoxelDataBlock *get_or_create_block_at_voxel_pos(VOX_Vector3i pos);
	VoxelDataBlock *create_default_block(VOX_Vector3i bpos);
...
private:
	...
	std::unordered_map<VOX_Vector3i, unsigned int> _blocks_map;
