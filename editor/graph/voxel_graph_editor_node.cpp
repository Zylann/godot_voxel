...
void VoxelGraphEditorNode::_notification(int p_what) {
	if (p_what == NOTIFICATION_DRAW) {
		if (_is_relay) {
			...
			const Vector2 input_pos = godot::get_graph_node_input_port_position(*this, 0);
			const Vector2 output_pos = godot::get_graph_node_output_port_position(*this, 0);
			draw_line(input_pos, output_pos, godot::get_graph_node_input_port_color(*this, 0), width, true);
		}
